...
function AddToCart() //add product or products to the cart
{
	...
	//stock control preventing adding product quantity 0 
        $avaiable_query = mysql_query("SELECT quantity FROM films WHERE id = '".$_POST['productcode']."' LIMIT 1;");
        if(!$avaiable_query) {
...
$HTML = ""; 
if ($itmcount == 0)
{ $HTML = "<h3>Your shopping cart is empty.</h3>"; 
} 
else
{ 
...
$HTML .= "<td>Total					</td>
 ...
 { 
 	...
 	$HTML .= "<td>".$cart[PRODUCTNAME][$i]."									
      ...
      $HTML .= "<td>".checkStock($cart[PRODUCTCODE][$i])."							
			  </td>"."\n"; 
 	$HTML .= "<td>
		<input 
...
// //number format for price
 	$HTML .= "<td>"."£".number_format($cart[PRICE][$i],2)."						
			  </td>"."\n"; 
	
	//number format for product total
	$HTML .= "<td>"."£".number_format($cart[PRICE][$i]*$cart[QUANTITY][$i],2)."	
			  </td>"."\n"; 
